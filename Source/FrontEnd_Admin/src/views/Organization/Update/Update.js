...
class Update extends Component {
  ...
  onHandleSubmit = (e) => {
    ...
    let organization = JSON.parse(JSON.stringify(this.state.organization));
    organization.image1 = create_name(this.state.file1.name);
    organization.image2 = create_name(this.state.file2.name);
    organization.image3 = create_name(this.state.file3.name);
    console.log(organization);
    axios({
      method: 'put',
      url:'http://127.0.0.1:8000/api/organization/'+id,
      data: organization
    })
      .then(res => {
    ...
    let url = 'http://127.0.0.1:8000/api/uploadFileOrganization';
  ...
  loadOrganizationInfor = () => {
    let id = this.props.match.params.id;
    fetch('http://localhost:8000/api/organization/' + id, {
      method: 'get',
    }).then(res => res.json()).then(json => {
      this.setState({
        isLoaded: true,
  ...
  handleContentChange1 = (event, editor) => {
    ...
    this.setState({organization: update(this.state.species, {'content1': {$set: data}})});
  };
  handleContentChange2 = (event, editor) => {
    const data = editor.getData();

    this.setState({organization: update(this.state.species, {'content2': {$set: data}})});
  };
  handleContentChange3 = (event, editor) => {
    const data = editor.getData();

    this.setState({organization: update(this.state.species, {'content3': {$set: data}})});
  };
  ...
  render() {
    let {items, isLoaded, organization} = this.state;
    if (!isLoaded) {
                  ...
                  <Form action="" method="post" encType="multipart/form-data" className="form-horizontal">
                    ...
                    <FormGroup row>
                      ...
                      <Col xs="12" md="9">
                        <Input placeholder="Nhập mô tả ngắn gọn" id="description_seo" name="description_seo"
                               value={organization.description_seo}
                               onChange={this.onHandleChange}/>
                      </Col>
                    </FormGroup>
                    <FormGroup row>
                    ...
                    <FormGroup row>
                      ...
                      <Col xs="12" md="9">
                        <CKEditor
                          ...
                          value={organization.content1}
                          data={organization.content1 || ''}
                          onInit={editor => {
                    ...
                    <FormGroup row>
                      ...
                      <Col xs="12" md="9">
                        <CKEditor
                          editor={ClassicEditor}
                          value={organization.content2}
                          data={organization.content2 || ''}
                          onInit={editor => {
                            this.setState({editor: editor})
                          }}
                    ...
                    <FormGroup row>
                      ...
                      <Col xs="12" md="9">
                        <CKEditor
                          editor={ClassicEditor}
                          value={organization.content3}
                          data={organization.content3 || ''}
                          onInit={editor => {
                            this.setState({editor: editor})
                          }}
